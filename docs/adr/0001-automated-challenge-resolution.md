...
Every browser action, login submission, consent action, and form submission carries a stable action identity, side-effect classification, idempotency condition, retry budget, and cancellation state. Before redispatch, Veilpick reconciles trusted post-condition evidence for that same action and resource. Idempotent actions may retry only within the remaining budget; non-idempotent actions never retry automatically unless reconciliation proves the prior attempt had no side effect. An ambiguous outcome returns a typed unresolved or failed disposition instead of repeating the action.
